  ...
  template: `
    <div class="card bg-base-100 shadow-lg">
      <div class="card-body p-4 flex flex-col items-center">
        <h3 class="card-title text-sm text-base-content mb-2 w-full">{{ title() }}</h3>
        <table class="charts-css pie mx-auto mb-3" [style]="tableStyle">
          <tbody>
            @for (item of data(); track item.label; let i = $index) {
              <tr>
                <td [style.--start]="getStart(i)" [style.--end]="getEnd(i)" [style.--color]="colors()[(i % colors().length)]"></td>
              </tr>
            }
          </tbody>
        </table>
          ...
          @for (item of data(); track item.label; let i = $index) {
            <div class="flex items-center gap-2 justify-between">
              <div class="flex items-center gap-2">
                <div class="w-2.5 h-2.5 rounded-sm flex-shrink-0" [style.backgroundColor]="colors()[(i % colors().length)]"></div>
                <span class="text-xs text-base-content">{{ item.label }}</span>
              </div>
              <span class="text-xs font-semibold text-base-content">{{ getPercentage(item.value) }}%</span>
            </div>
          }
        </div>
      </div>
